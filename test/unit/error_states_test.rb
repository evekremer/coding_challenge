...
class ErrorStatesTest < BaseTest
    ...
    def test_empty_command_1
        socket.puts "\r\n"
        socket.puts "value\r\n"
        assert_equal Memcached::Util::INVALID_COMMAND_NAME_MSG, socket.gets
    end
    ...
    def test_key_too_long
        key = "k" * (Memcached::Util::MAX_KEY_LENGTH + 1)

        send_storage_cmd("set", key, 4, 6230, value.length(), false, value, false)
        assert_equal "CLIENT_ERROR <key> has more than #{Memcached::Util::MAX_KEY_LENGTH} characters\r\n", socket.gets
    ...
    def test_value_too_long
        value = "v" * (Memcached::Util::MAX_VALUE_LENGTH + 1)
